...
整合内容:
1. ACmix-Swin 深度学习分类器
2. Python实现的WGCNA风格共表达网络分析
3. 生成与R可视化脚本(V9.2)兼容的输出文件

输出文件 (./Input/):
- node.xlsx, edge.xlsx, layout.xlsx  (网络结构)
- module_correlation_matrix.csv      (模块间相关性)
- module_pvalue_matrix.csv           (模块间P值)
- module_trait_correlation.csv       (模块-表型相关性)
- module_trait_pvalue.csv            (模块-表型P值)
- WGCNA_results.pkl                  (完整WGCNA结果)
- metabolite_types.xlsx              (基因类型)
...
class NetworkFileGenerator:
    ...
    def generate_hub_genes(self, gene_importance_df, wgcna_results, 
        ...
        Parameters:
        -----------
        n_phenotype_hub : int or dict
            Number of hub genes per phenotype.
            - int: same number for all phenotypes
            - dict: {class_name: n_hub} for each phenotype, e.g. {'Drone': 15, 'Queen': 10, 'Worker': 12}
        dl_weight : float
            Weight for DL importance (0.0-1.0). Default=1.0 means 100% DL.
            WGCNA weight = 1.0 - dl_weight
        """
        print("\n[Hub Gene Selection]")
        print(f"  Scoring weights: DL={dl_weight:.0%}, WGCNA={1-dl_weight:.0%}")
        
        # 处理 n_phenotype_hub 参数 - 支持int或dict
        if isinstance(n_phenotype_hub, dict):
            hub_counts = n_phenotype_hub
            print(f"  Per-class hub counts: {hub_counts}")
        else:
            # 统一数量
            hub_counts = {name: n_phenotype_hub for name in class_names}
        ...
        # Combined score (可调整权重)
        wgcna_weight = 1.0 - dl_weight
        scores_df['combined_score'] = dl_weight * scores_df['dl_score'] + wgcna_weight * scores_df['wgcna_score']
        ...
        # Phenotype-specific hub genes (完全基于DL的分类别重要性)
        phenotype_hub = {}
        for i, class_name in enumerate(class_names):
            # 获取该类别的hub数量
            n_hub = hub_counts.get(class_name, 10)  # 默认10
            
            # 使用DL的分类别重要性
            class_col = f'{class_name}_importance'
            if class_col in gene_importance_df.columns:
                class_importance = gene_importance_df.set_index('gene')[class_col]
                class_importance = (class_importance - class_importance.min()) / (class_importance.max() - class_importance.min() + 1e-8)
                scores_df[f'score_{class_name}'] = scores_df['gene'].map(class_importance).fillna(0)
            else:
                # Fallback: 使用总体DL分数
                scores_df[f'score_{class_name}'] = scores_df['dl_score']
            
            top_genes = scores_df.nlargest(n_hub, f'score_{class_name}')['gene'].tolist()
    ...
    def generate_node_edge_layout(self, overall_hub, phenotype_hub, 
        ...
        group_y = np.linspace(y_positions[0] - 5, y_positions[-1] + 5, len(class_names))
        for i, class_name in enumerate(class_names):
            layouts.append({
...
# ============================================================================
# WGAN-GP (from original code - abbreviated)
# ============================================================================
...
class WGAN_GP:
    ...
    def train(self, real_data, labels, epochs=800, batch_size=8,
        ...
        for epoch in range(epochs):
            epoch_g_loss, epoch_c_loss = 0, 0
            epoch_w_dist, epoch_gp = 0, 0
            ...
            for real_samples, real_labels in dataloader:
                ...
                # Store last critic iteration values for logging
                last_real_score, last_fake_score, last_gp = 0, 0, 0
                ...
                for _ in range(self.n_critic):
                    ...
                    last_real_score = real_score.mean().item()
                    last_fake_score = fake_score.mean().item()
                    last_gp = gp.item()
                
                self.g_optimizer.zero_grad()
                noise = torch.randn(batch_size_actual, self.noise_dim).to(self.device)
                fake_samples = self.generator(noise, real_labels)
                fake_score = self.critic(fake_samples, real_labels)
                ...
                epoch_w_dist += (last_real_score - last_fake_score)  # Wasserstein距离估计
                epoch_gp += last_gp
            ...
            self.history['c_loss'].append(epoch_c_loss / n_batches)
            self.history['w_distance'].append(epoch_w_dist / n_batches)
            self.history['gp'].append(epoch_gp / n_batches)
            self.history['real_score'].append(epoch_real_score / n_batches)
            self.history['fake_score'].append(epoch_fake_score / n_batches)
            
            if (epoch + 1) % 200 == 0:
                print(f"  Epoch {epoch+1}/{epochs}: G_Loss={self.history['g_loss'][-1]:.4f}, "
                      f"W_Dist={self.history['w_distance'][-1]:.4f}")
...
# ============================================================================
# Data Augmenter (with full statistics)
# ============================================================================
...
# ============================================================================
# ACmix-Swin Classifier (abbreviated)
# ============================================================================
...
def plot_data_distribution(X, y, class_names, title, save_path):
    set_sci_style()
    ...
    colors = [SCI_COLORS['drone'], SCI_COLORS['queen'], SCI_COLORS['worker']]
...
def plot_gene_importance_by_class(class_importance_df, class_names, top_n=20, save_path='gene_importance_by_class.pdf'):
    set_sci_style()
    
    fig, axes = plt.subplots(1, len(class_names), figsize=(5*len(class_names), 8))
    ...
    for idx, class_name in enumerate(class_names):
        ...
        y_pos = np.arange(actual_n)
        color = colors[idx] if idx < len(colors) else plt.cm.tab10(idx)
    ...
    plt.suptitle('Top Genes by Phenotype', fontweight='bold', fontsize=14, y=1.02)
    plt.tight_layout()
    plt.savefig(save_path, format='pdf', bbox_inches='tight')
    plt.close()
    print(f"  Saved: {save_path}")
...
def plot_gene_heatmap(sample_gene_contrib, y, gene_names, class_names, top_n=30, save_path='gene_heatmap.pdf'):
    set_sci_style()
    ...
    fig, ax = plt.subplots(figsize=(12, max(6, actual_n * 0.25)))
    ...
    for i, (boundary, name) in enumerate(zip(class_boundaries, class_names)):
        mid = (prev + boundary) / 2
        ax.text(mid, -1.5, name, ha='center', va='top', fontweight='bold', fontsize=10)
        prev = boundary
...
def load_data(expr_path, sample_file=None):
    ...
    print(f"  Sample columns in expression matrix (first 5): {all_samples[:5]}")
    ...
    if sample_file:
        ...
        with open(sample_file, 'r') as f:
            for line in f:
                ...
                if len(parts) >= 2:
                    sample_info.append((parts[0].strip(), parts[1].strip()))
        
        print(f"  Sample file entries (first 5): {sample_info[:5]}")
        
        if not sample_info:
            raise ValueError("No valid entries found in sample file! "
                           "Expected format: class<TAB>sample_name or sample_name<TAB>class")
        
        # 自动检测格式: class<TAB>sample 还是 sample<TAB>class
        # 检查第一列还是第二列匹配表达矩阵的列名
        first_col_matches = sum(1 for x in sample_info if x[0] in all_samples)
        second_col_matches = sum(1 for x in sample_info if x[1] in all_samples)
        
        print(f"  Format detection: first_col_matches={first_col_matches}, second_col_matches={second_col_matches}")
        
        if second_col_matches > first_col_matches:
            # 格式是: class<TAB>sample_name (原始假设)
            print("  Detected format: class<TAB>sample_name")
            class_names = list(dict.fromkeys([x[0] for x in sample_info]))
            class_to_idx = {name: idx for idx, name in enumerate(class_names)}
            
            valid_samples, valid_labels = [], []
            for class_name, sample_name in sample_info:
                if sample_name in all_samples:
                    valid_samples.append(sample_name)
                    valid_labels.append(class_to_idx[class_name])
        else:
            # 格式是: sample_name<TAB>class (反过来的)
            print("  Detected format: sample_name<TAB>class")
            class_names = list(dict.fromkeys([x[1] for x in sample_info]))
            class_to_idx = {name: idx for idx, name in enumerate(class_names)}
            
            valid_samples, valid_labels = [], []
            for sample_name, class_name in sample_info:
                if sample_name in all_samples:
                    valid_samples.append(sample_name)
                    valid_labels.append(class_to_idx[class_name])
        
        print(f"  Matched samples: {len(valid_samples)} / {len(sample_info)}")
        
        if len(valid_samples) == 0:
            print("\n" + "!" * 60)
            print("ERROR: No samples matched!")
            print("!" * 60)
            print(f"\nSample names in expression matrix (first 10):")
            for s in all_samples[:10]:
                print(f"  '{s}'")
            print(f"\nSample names in sample file (first 10):")
            for entry in sample_info[:10]:
                print(f"  '{entry[0]}' | '{entry[1]}'")
            print("\nPlease check:")
            print("  1. Sample names must match exactly (case-sensitive)")
            print("  2. Sample file format: class<TAB>sample_name or sample_name<TAB>class")
            raise ValueError("No samples matched between expression matrix and sample file!")
        
        X = expr_df[valid_samples].values.T
        y = np.array(valid_labels)
        sample_to_group = {s: class_names[l] for s, l in zip(valid_samples, valid_labels)}
        
    else:
        # 无样本文件时，从列名自动推断
        print("  No sample file provided, inferring labels from column names...")
        labels = []
    ...
    print(f"\nFinal data shape: {X.shape[0]} samples x {X.shape[1]} genes")
    print(f"Classes: {class_names}")
    print(f"Sample distribution: {dict(zip(class_names, [np.sum(y == i) for i in range(len(class_names))]))}")
    
    if X.shape[0] == 0:
        raise ValueError("No valid samples loaded! Please check your input files.")
...
def main(expr_path, output_dir='./output_wgcna', sample_file=None,
         ...
         n_overall_hub=20, n_phenotype_hub=10):
    """
    Main pipeline with WGCNA integration.
    ...
    output_dir : str
        Output directory
    sample_file : str
        Sample grouping file (format: class<TAB>sample_name)
    n_features : int
        Number of features to select
    samples_per_class : int
        Target samples per class after augmentation
    gan_epochs : int
        WGAN-GP training epochs
    embed_dim : int
    ...
    dropout : float
        Dropout rate
    epochs : int
        Max classifier training epochs
    batch_size : int
        Training batch size
    learning_rate : float
    ...
    use_mixup : bool
        Whether to use mixup during training
    mixup_alpha : float
        Mixup alpha parameter
    label_smoothing : float
        Label smoothing factor
    n_overall_hub : int
        Number of overall hub genes
    n_phenotype_hub : int or dict
        Number of hub genes per phenotype.
        - int: same number for all phenotypes (e.g., 10)
        - dict: per-class counts (e.g., {'Drone': 15, 'Queen': 10, 'Worker': 12})
    """
    ...
    # 合并分类别重要性到 importance_df
    for class_name in class_names:
        col_name = f'{class_name}_importance'
        if col_name in class_importance_df.columns:
            importance_df = importance_df.merge(
                class_importance_df[['gene', col_name]], 
                on='gene', 
                how='left'
            )
    ...
    # Hub gene selection (100% 基于DL重要性)
    ...
    # Gene importance by class (已在前面计算过)
    ...
    # Gene heatmap
    # Compute sample gene contribution
    model.eval()
    ...
    print(f"\nOutput files:")
    print(f"  {output_dir}/")
    print(f"    ├── Input/                         # R可视化脚本输入文件")
    print(f"    │   ├── node.xlsx")
    print(f"    │   ├── edge.xlsx")
    print(f"    │   ├── layout.xlsx")
    print(f"    │   ├── module_correlation_matrix.csv")
    print(f"    │   ├── module_pvalue_matrix.csv")
    print(f"    │   ├── module_trait_correlation.csv")
    print(f"    │   ├── module_trait_pvalue.csv")
    print(f"    │   ├── groups.csv")
    print(f"    │   ├── metabolite_types.xlsx")
    print(f"    │   └── WGCNA_results.pkl")
    print(f"    │")
    print(f"    ├── Visualizations (PDF)")
    print(f"    │   ├── training_curves.pdf        # 训练曲线")
    print(f"    │   ├── wgan_training.pdf          # WGAN训练过程")
    print(f"    │   ├── confusion_matrix.pdf       # 混淆矩阵")
    print(f"    │   ├── acmix_weights.pdf          # ACmix权重")
    print(f"    │   ├── augmentation_summary.pdf   # 数据增强统计")
    print(f"    │   ├── data_original.pdf          # 原始数据PCA")
    print(f"    │   ├── data_augmented.pdf         # 增强数据PCA")
    print(f"    │   ├── data_comparison.pdf        # 数据对比")
    print(f"    │   ├── distance_analysis.pdf      # 距离分析")
    print(f"    │   ├── gene_importance.pdf        # 基因重要性")
    print(f"    │   ├── gene_importance_by_class.pdf  # 分类别重要性")
    print(f"    │   ├── gene_heatmap.pdf           # 基因热图")
    print(f"    │   ├── wgcna_module_trait.pdf     # 模块-表型热图")
    print(f"    │   └── wgcna_module_correlation.pdf  # 模块间相关性")
    print(f"    │")
    print(f"    ├── Data Files (CSV)")
    print(f"    │   ├── training_history.csv")
    print(f"    │   ├── gene_importance.csv")
    print(f"    │   ├── gene_importance_by_class.csv")
    print(f"    │   ├── sample_gene_contribution.csv")
    print(f"    │   ├── gene_scores_combined.csv")
    print(f"    │   ├── predictions.csv")
    print(f"    │   └── selected_features.csv")
    print(f"    │")
    print(f"    └── model.pth                      # 模型权重")
    print(f"\n★ 运行R可视化脚本:")
    print(f"  1. 将 {input_dir}/ 复制到R脚本的 ./Input/ 目录")
    print(f"  2. 运行可视化脚本 V9.2")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='ACmix-Swin + WGCNA Integration',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Example Usage:
  # 基本用法
  python acmix_swin_wgcna.py --expr exp.csv --samples samples.txt
  ...
  # 完整参数示例
  python acmix_swin_wgcna.py --expr exp.csv --samples samples.txt \\
      --output ./results --n_features 100 --samples_per_class 50 \\
      --embed_dim 64 --num_heads 4 --window_size 7 \\
      --dropout 0.3 --lr 1e-4 --epochs 300 --patience 50
  ...
  # 自定义每组Hub基因数量 (分别指定)
  python acmix_swin_wgcna.py --expr exp.csv --samples samples.txt \\
      --n_overall_hub 30 --n_phenotype_hub "Drone:20,Queen:15,Worker:18"
        '''
    )
    
    # 数据参数
    parser.add_argument('--expr', type=str, required=True, help='Expression matrix CSV path')
    parser.add_argument('--samples', type=str, default=None, help='Sample grouping file')
    ...
    parser.add_argument('--n_phenotype_hub', type=str, default='10', 
                        help='Hub genes per phenotype. Can be: '
                             '(1) Single number for all: "10" '
                             '(2) Per-class format: "Drone:15,Queen:10,Worker:12"')
    
    args = parser.parse_args()
    
    # 解析 n_phenotype_hub 参数
    def parse_phenotype_hub(value):
        """解析表型hub基因数量参数"""
        value = value.strip()
        # 尝试作为单个数字解析
        try:
        ...
        # 解析 "Class1:N1,Class2:N2" 格式
        hub_dict = {}
        for item in value.split(','):
            item = item.strip()
            if ':' in item:
                class_name, count = item.split(':')
                hub_dict[class_name.strip()] = int(count.strip())
        
        if hub_dict:
            return hub_dict
        else:
            raise ValueError(f"Cannot parse n_phenotype_hub: {value}. "
                           f"Use '10' or 'Drone:15,Queen:10,Worker:12' format.")
    
    n_phenotype_hub = parse_phenotype_hub(args.n_phenotype_hub)
    ...
    main(
        ...
        n_phenotype_hub=n_phenotype_hub
    )
